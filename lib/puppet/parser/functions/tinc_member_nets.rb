...
This function takes two arguments (tinc_nets hash, and the node's fqdn_tinc)
...
EOF
  ...
  fqdn_tinc = args.to_a[1]

  raise Puppet::ParseError, "First argument must be a hash" unless tinc_nets.is_a?(Hash)
  raise Puppet::ParseError, "Second argument must be a string" unless fqdn_tinc.is_a?(String)
  ...
  member_nets = tinc_nets.keys.select { |key| tinc_nets[key]['member_nodes'].has_key?(fqdn_tinc) }
